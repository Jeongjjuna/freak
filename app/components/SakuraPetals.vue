...
onMounted(() => {
  ...
  petals.value = Array.from({ length: 35 }, (_, i) => ({
    id: i,
...
<template>
  <div v-if="isSakura && petals.length > 0" class="fixed inset-0 pointer-events-none z-40 overflow-hidden">
    <div
      v-for="p in petals"
      :key="p.id"
      :style="{
