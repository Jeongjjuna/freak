...
상태 설계의 첫 번째 원칙입니다. 핵심은 `읽기 전용`과 `불변`이 다르다는 점입니다.

### 읽기 전용인데 불변이 아니다?
`List` 타입은 수정 메서드만 없을 뿐, 같은 객체를 가리키는 가변 참조가 값을 바꾸면 함께 바뀝니다.
```kotlin
...
### 불변성을 보장하는 2가지 방법
1. 반환할 때는 `List` 타입으로 반환합니다(가변 참조를 밖에 넘기지 않기). 내부 리스트라면 복사본을 반환합니다.
```kotlin
fun getList(): List<Int> = mutableList.toList()
```
2. 받은 인수는 복사해서 저장합니다(방어적 복사). 그렇지 않으면 호출한 쪽이 내부 값을 변경할 수 있습니다.
```kotlin
fun setList(newList: List<Int>) { list = newList.toList() }
```

원칙은 `가변 객체 참조를 외부에 직접 내놓지 말 것`입니다. 어떤 영향을 줄지 예측할 수 없기 때문입니다.

### 부분적 불변성
함께 바뀌는 변수끼리 묶어서, "필드를 하나씩 고치기" 대신 "묶음을 통째로 새 객체로 교체"하게 만듭니다.
...
- 따로 관리할 가변 필드가 5개에서 참조 2개로 줄어 관리 포인트가 감소합니다.
- 함께 바뀔 값들이 항상 한 덩어리로 교체되므로, `이름만 바뀌고 이메일은 옛날 값`인 어중간한 상태가 생기지 않습니다.
- 묶음 내부는 `val`(불변)이라 한 번 만든 `UserProfile`은 절대 변하지 않습니다. 바뀌는 것은 "어떤 묶음을 가리키느냐"(참조)뿐입니다.

> `핵심` — 읽기 전용 타입이라도 가변 참조가 살아있으면 불변이 보장되지 않습니다. 반환·저장은 복사로 하고, 가변 참조는 밖에 내놓지 않아야 합니다.
