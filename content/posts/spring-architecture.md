...
레이어드 아키텍처의 핵심은 의존 방향을 단방향으로 유지하여 변경의 영향을 제한하는 것입니다.
...
여러 도메인이 동시에 의존하는 공용 서비스가 되어버렸습니다.
...
아키텍처 규칙을 사람이 계속 기억하지 않아도 테스트가 대신 확인해준다는 점이었습니다.
...
세 가지 방법을 시도해보면서 공통적으로 느낀 점이 있습니다.

아키텍처 규칙을 지키는 것은 팀원의 의지나 숙련도의 문제가 아니라,
시스템이 얼마나 일관되게 강제하고 있는가의 문제라고 느꼈습니다.

바쁜 일정 속에서, 마감 압박이 있는 상황에서,
혹은 새로 합류한 팀원이 충분한 컨텍스트 없이 작업하게 되는 순간에도
구조가 무너지지 않으려면 규칙은 사람의 기억이 아니라 시스템에 의해 유지되어야 합니다.

이번 경험에서 얻은 핵심 인사이트를 정리하면 다음과 같습니다.

1. **구조가 먼저 정리되어 있어야 강제도 가능하다.** DTO와 레이어 경계가 섞여 있다면 어떤 도구를 써도 결국 사람의 주의에 의존하게 됩니다.
2. **팀 상황에 맞게 선택해야 한다.** 가장 확실한 방법이 항상 최선은 아닙니다. 도입 비용과 팀 숙련도를 고려해 점진적으로 적용하는 것이 현실적이라고 생각합니다.
3. **UseCase 명세는 사람과 AI 모두에게 좋은 작업 경계가 된다.** 사람이든 AI든 “어디까지 구현해야 하는지”가 명확해지면 설계가 쉽게 흔들리지 않습니다.

Agentic Coding 시대에 아키텍처 검증 역시 점점 자동화의 영역으로 이동하고 있습니다.
앞으로는 UseCase 명세가 변경될 때 검증 규칙까지 함께 동기화되는 구조를 더 다듬어볼 계획입니다.

좋은 아키텍처는 문서로 유지되지 않습니다.
코드가 작성되는 순간부터 빌드와 테스트 과정까지,
지속적으로 검증되고 강제될 때만 살아남습니다.

규칙은 강제될 때 비로소 규칙이 됩니다.

이 글이 비슷한 고민을 하시는 분들께 작은 힌트가 되길 바랍니다.