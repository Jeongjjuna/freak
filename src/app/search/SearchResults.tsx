  ...
  return (
    <>
      <div className="flex items-center gap-2 mb-7 text-[13px] text-[#737373]">
        {trimmed ? (
          <>
            <span className="font-bold text-[#3a4954]">&ldquo;{trimmed}&rdquo;</span>
            <span>검색 결과</span>
          </>
        ) : (
          <span className="font-bold text-[#3a4954]">전체 게시글</span>
        )}
        <span className="text-[12px] border border-[#e5e5e5] px-2 py-0.5 rounded-full bg-[#f8f8f8]">
      ...
      {pagePosts.length > 0 ? (
        ...
        <p className="text-[15px] text-[#737373] py-10 text-center">
          검색 결과가 없습니다.
        </p>
      )}
      {!trimmed && (
